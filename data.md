...
**Directories:** 29 CvM-fitted distributions, each in its own subdirectory under `data/fitted/`.
...
ADC alternating DNL power comparison results (n=10). Contains per-epsilon, per-N power values for CT (exact), CT (MC+gamma), Pearson's chi-squared, and G-test.
