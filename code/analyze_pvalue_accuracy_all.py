#!/usr/bin/env python3
"""Full p-value accuracy analysis across ALL available (N,n) pairs, parallelized."""
...
def main():
    ...
    with mp.Pool(n_workers) as pool:
        for r in pool.imap_unordered(process_pair, all_pairs, chunksize=256):
            done += 1
            if done % 20000 == 0:
    ...
    # Save full JSON
    output = {'total_cdf_files': len(all_pairs), 'valid_pairs': len(results_all), 'skipped': skipped}
    ...
    with open('pvalue_accuracy_all_results.json', 'w') as f:
        json.dump(output, f, indent=2)
    print("\nSaved: pvalue_accuracy_all_results.json")
